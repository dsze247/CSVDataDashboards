...
function FileParsing(props) {
    ...
    const handleFileUpload = (event) => {   // sets the file to parse to the newly inputted file and clears the currently stored data
        setFile(event.target.files[0]);
        props.onValueChange([[], []]);
    }

    const handleFileParse = (event) => { // checks for errors, parses through the inputed file, sets new data and headers
        if (!file) {
        ...
        Papa.parse(file, {
            ...
            complete: (results) => {
                ...
                const hasValidColumns = results.meta.fields.length === expectedColumns.length &&
                                        results.meta.fields.every((h, i) => h.toLowerCase() === expectedColumns[i]);
